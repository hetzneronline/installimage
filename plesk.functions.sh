...
install_plesk() {
  ...
  curl --location --output ${FOLD}/hdd/${temp_file} --silent --write-out '%{response_code}' ${PLESK_INSTALLER_SRC}/${IMAGENAME} | grep --quiet 200 || return 1
  ...
  if ! echo "${version}" | egrep --quiet "^PLESK_[[:digit:]]+_[[:digit:]]+_[[:digit:]]+$"; then
    plesk_version=$(execute_chroot_command_wo_debug "${temp_file} --select-product-id plesk --show-releases 2> /dev/null" \
      | tail -n +2 \
      | grep "^${version}" \
      | head -n 1 \
      | awk '{ print $2 }')
  ...
  local command="${temp_file} --select-product-id plesk --select-release-id ${version} --download-retry-count ${PLESK_DOWNLOAD_RETRY_COUNT} ${PLESK_COMPONENTS[@]/#/--install-component }"
